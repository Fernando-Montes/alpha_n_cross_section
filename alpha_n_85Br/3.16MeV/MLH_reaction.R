...
beam_ratio = (307721300/4.51)/(153396900/2.51) # ratio of beam intensity between gas in and gas out runs
...
factorCS = 4073708/(307721300*5.93e-8) # convert rate to mbarn

a1_mean = 185/factorCS
da1 =     70/factorCS
a2_mean = 20/factorCS
da2 =     20/factorCS

tempNorm = 1/3.0e1
prob_observed( 0.0007844507, 2.078844e-05, 4.030811e-06, 7.575845e-06, 1.040776e-06, 0, 0, 0.000700, 0.000100, 1,  1, 0.22 )
...
fileName <- "~/Dropbox/Courses/R/alpha_n/alpha_n_85Br/3.16MeV/prob_dist_beam.RData"
load(file = fileName)
...
ptime <- system.time({
  ...
  table <- foreach(k=1:10000, .combine = rbind, .verbose = F, .errorhandling = "remove") %dopar% { 
    ...
    a1 = runif(1, 1*(a1_mean-da1), a1_mean+da1)
    # a1 = 0
    prob_dist[1, "a1"] = a1
    a2 = runif(1, 1*(a2_mean-da2), a2_mean+da2)
    # a2 = 0
    prob_dist[1, "a2"] = a2
    
    prob_dist[1, "effn"] = effn
    prob_dist[1, "effb"] = effb
    effr = runif(1, 0.17, 0.3)
    #effr = 0.22
