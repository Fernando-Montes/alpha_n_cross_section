...
tempNorm = 1/1.2e6
prob_observed(0.002235807, 3.086643e-05, 0.0001624678, 2.922833e-05, 1.294953e-05, 0, 0, 1.0, 1.0)

# starting values n1, n2, eff....
fileName <- "~/Dropbox/Courses/R/alpha_n/2.97MeV/prob_dist_room.RData"
load(file = fileName)
...
ptime <- system.time({
  ...
  table <- foreach(k=1:100000, .combine = rbind, .verbose = F, .errorhandling = "remove") %dopar% { 
...
fileName <- "~/Dropbox/Courses/R/alpha_n/2.97MeV/prob_dist_beam.RData"
save(prob_dist, file = fileName)

load("/Users/fernandomontes/Dropbox/Courses/R/alpha_n/2.97MeV/prob_dist_beam.RData")
