...
fileName <- "~/Dropbox/Courses/R/alpha_n/prob_dist_beam_Hendrik.RData"
load(file = fileName)

l = 894200  # number of time-windows

ptime <- system.time({
  ind = rownames(prob_dist)
  table <- foreach(k=1:80, .combine = rbind, .verbose = F, .errorhandling = "remove") %dopar% { 
    indx = sample(ind,1)
    eff = prob_dist[indx,"effn"]
    n1 =   prob_dist[indx,"n1"]
    n2 =   prob_dist[indx,"n2"]
    b1 =   prob_dist[indx,"b1"]
    b2 =   prob_dist[indx,"b2"]
    b3 =   prob_dist[indx,"b3"]
    b4 =   prob_dist[indx,"b4"]
    b5 =   prob_dist[indx,"b5"]
