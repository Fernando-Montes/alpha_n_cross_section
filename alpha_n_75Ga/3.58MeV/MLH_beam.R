...
tempNorm = 1/1.08e5
prob_observed(0.002275502, 2.875796e-05, 0.001374947, 0.0004969205, 9.056707e-05, 0, 0, 1.0, 1.0)

# starting values n1, n2, eff....
fileName <- "~/Dropbox/Courses/R/alpha_n/3.58MeV/prob_dist_room.RData"
load(file = fileName)
...
ptime <- system.time({
  ...
  table <- foreach(k=1:30000, .combine = rbind, .verbose = F, .errorhandling = "remove") %dopar% { 
...
fileName <- "~/Dropbox/Courses/R/alpha_n/3.58MeV/prob_dist_beam.RData"
save(prob_dist, file = fileName)

load("/Users/fernandomontes/Dropbox/Courses/R/alpha_n/3.58MeV/prob_dist_beam.RData")
