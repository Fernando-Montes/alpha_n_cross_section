...
# Function returning probability of detecting det number of neutrons in time-window 
# coming from source producing 2 neutrons but with two different detection efficiencies
prob_det_sour_eff = function(det, rate, eff1, eff2) {
  prob = 0
  for(i in ceiling(det/2):10) {  # number of events
    total = 2*i                  # total number of emitted neutrons
    emitted_eff_max = total/2
    prob_temp = 0
    total_terms = 0
    for(d1 in max(0,det-emitted_eff_max):min(det, emitted_eff_max)) {
      nd1 = emitted_eff_max-d1
      d2 = det - d1
      nd2 = emitted_eff_max-d2
      total_terms = total_terms + comb(det,d1)*comb(total-det,nd1)
      prob_temp = prob_temp + dpois(i, rate)*eff1^d1*eff2^d2*(1-eff1)^nd1*(1-eff2)^nd2*factorial(total)/(factorial(d1)*factorial(d2)*factorial(nd1)*factorial(nd2))
    }
    prob = prob + prob_temp/total_terms
  }
  return( prob )
}

# Function returning probability of detected det number of neutrons in time-window
prob_det = function(det, n1, n2,  b1, b2, b3, b4, b5, a1, a2, effn, effb, effr1, effr21, effr22){
  probTemp = 0
  for (n1det in det:0) {
              ...
              for (b5det in (det-n1det-n2det-b1det-b2det-b3det-b4det):0) {
                for (a1det in (det-n1det-n2det-b1det-b2det-b3det-b4det-b5det):0) {
                  ...
                  probTemp = probTemp + 
                    ...
                    prob_det_sour(a1det, 1, a1, effr1)*
                    prob_det_sour_eff(a2det, a2, effr21, effr22)
                }
              }
            }
          }
...
prob_observed = function(n1, n2, b1, b2, b3, b4, b5, a1, a2, effn, effb, effr1, effr21, effr22) {
  p0 = prob_det(0, n1, n2, b1, b2, b3, b4, b5, a1, a2, effn, effb, effr1, effr21, effr22)
  p1 = prob_det(1, n1, n2, b1, b2, b3, b4, b5, a1, a2, effn, effb, effr1, effr21, effr22)
  p2 = prob_det(2, n1, n2, b1, b2, b3, b4, b5, a1, a2, effn, effb, effr1, effr21, effr22)
  p3 = prob_det(3, n1, n2, b1, b2, b3, b4, b5, a1, a2, effn, effb, effr1, effr21, effr22)
  prob_total = 1
  for (i in 1:norma) {
...
tempNorm = 1/37.85
prob_observed(0.002178224, 3.006295e-05, 0.001580331, 0.0005316282, 9.688735e-05, 0,  0, 0.004133199, 0.0001940544, 1.0, 1.0, 0.2396504, 0.2396504, 0.2396504)

# starting values n1, n2, eff....
fileName <- "~/Dropbox/Courses/R/alpha_n/4.02MeV/prob_dist_beam.RData"
load(file = fileName)
...
a1_mean = 1200/factorCS
da1 =     700/factorCS
a2_mean = 300/factorCS
da2 =     300/factorCS

ptime <- system.time({
  
  ind = rownames(prob_dist_beam)
  table <- foreach(k=1:300000, .combine = rbind, .verbose = F, .errorhandling = "remove") %dopar% { 
    prob_dist = data.frame(
      ...
      effr1 = numeric(0),    # neutron detection efficiency reaction-induced (a,1n) neutrons
      effr21 = numeric(0),    # neutron detection efficiency reaction-induced (a,2n) neutrons
      effr22 = numeric(0),    # neutron detection efficiency reaction-induced (a,2n) neutrons
    ...
    # a2 = 0
    prob_dist[1, "a2"] = a2
    effr1 = runif(1, 0.198, 0.245)
    effr21 = runif(1, 0.186, 0.208)
    effr22 = runif(1, 0.103, 0.154)
    #effr = 0.22
    prob_dist[1, "effr1"] = effr1
    prob_dist[1, "effr21"] = effr21
    prob_dist[1, "effr22"] = effr22
    
    prob_dist[1, "prob"] = prob_observed(n1, n2, b1, b2, b3, b4, b5, a1, a2, effn, effb, effr1, effr21, effr22)
...
fileName <- "~/Dropbox/Courses/R/alpha_n/4.02MeV/prob_dist_reaction2eff.RData"
...
(prob_table == max(prob_table)) == TRUE
